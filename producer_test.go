...
// produce ждёт теста.
//
// ОБЯЗАТЕЛЬНОЕ УСЛОВИЕ ВЫЗЫВАЮЩЕГО: release нужно закрыть ЗАДОЛГО до того, как
// погаснет кластер, а не соседним t.Cleanup рядом с его закрытием. В kfake есть
// гонка, которая при закрытии кластера поверх ещё спящей управляющей функции
// вешает горутину цикла кластера навсегда: SleepControl кладёт каналы в
// буферизованный controlSleep и лишь потом встаёт в select с die; если die уже
// закрыт, он возвращается сразу, горутина функции берёт controlMu и шлёт
// ответ, а в tryControlKey готовы обе ветки — и выбор ветки controlSleep ведёт
// в beginSleptControl за тем же самым controlMu. Отпустить его мог бы только
// finishSleptControl, до которого цикл уже не дойдёт.
//
// Стоит это дорого и ловится плохо: примерно одно срабатывание на несколько
// сотен, обычными прогонами (в том числе -count=60 и -race) не воспроизводится
// вовсе, а проявляется падением goleak в TestMain — «тест прошёл, прогон
// упал», без всякой связи с виновником. Один такой тест из-за этого уже
// переписан (см. TestProducerCloseFlushTimesOut). Тесты ниже безопасны по
// построению: оба закрывают release посреди сценария, и до гашения кластера
// успевает пройти настоящая работа.
