...
Два изменения: механическое переименование типов и одно изменение поведения.

### Изменение поведения: умолчание `Producer.MaxRetries`

Было `3`, стало `-1` — «повторять без ограничения». Единственное место, где v2
ведёт себя иначе при той же конфигурации, и молчать о нём нельзя: конечный
лимит открывает тихую потерю подтверждённой записи при смене лидера партиции
(см. [«Подтверждённая запись при смене лидера»](#подтверждённая-запись-при-смене-лидера)).
...
Единственное изменение поведения во всей миграции — умолчание `MaxRetries`
выше, и компилятор о нём не скажет: конфигурация остаётся валидной.
