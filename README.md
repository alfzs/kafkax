...
Путь модуля теперь `github.com/alfzs/kafkax/v3`, поэтому импорт правится в любом
случае. Дальше маршрута два:

* **с v1** (`github.com/alfzs/kafkax`) — сменились транспорт
  (confluent-kafka-go → franz-go), имена типов, сигнатуры конструкторов, а
  семнадцать ключей конфигурации исчезли под прежними именами;
* **с v2.0.0** (`github.com/alfzs/kafkax/v2`) — версия прожила часы, но она
  выпущена и закеширована прокси-модулями. Ломающих изменений три, все
  локальные.

Начинать в обоих случаях стоит с конфигурации: это единственная часть перехода,
которую компилятор не проверяет вовсе.

### Отставленные ключи: перенесённый конфиг падает, а не молчит

cleanenv игнорирует и незнакомый yaml-ключ, и незнакомую переменную окружения.
Перенесённый как есть конфиг от этого не сломался бы — он тихо потерял бы
настройку и взял умолчание. Разницу между «настройка потерялась» и «настройка
сменила единицу измерения» видно не по логам, а по потолку памяти в десятки
гигабайт.

Поэтому в v3 старые имена не исчезли, а стали отказом на старте. Ключ в файле
роняет разбор `Config`, ключ в окружении — валидацию, то есть оба конструктора и
`Validate`. Претензии собираются все разом, и каждая называет замену и суть
изменения:

```
kafkax: invalid config file: consumer.message_queue_size is retired: renamed to consumer.message_queue_batches (env KAFKAX_CONSUMER_MESSAGE_QUEUE_BATCHES) and the unit changed from messages to batches: v1 buffered single messages, v3 buffers whole poll batches, so a value carried over as is raises the memory ceiling (partitions x batches x max_partition_bytes) by up to max_poll_records times; pick a batch count, the default is 100
consumer.handler_max_retries is retired: renamed to consumer.handler_retries (env KAFKAX_CONSUMER_HANDLER_RETRIES) and the meaning of 0 is inverted: in v1 zero meant retry forever and the value counted handler calls, in v3 zero means no retries at all, N means N retries on top of the first call and -1 means retry forever
```

Тот же ключ, заданный переменной окружения, приезжает претензией валидации:

```
kafkax: invalid config: KAFKAX_CONSUMER_MESSAGE_QUEUE_SIZE is retired: renamed to consumer.message_queue_batches (env KAFKAX_CONSUMER_MESSAGE_QUEUE_BATCHES) and the unit changed from messages to batches: ...
```

Секция ключа роли не играет: продюсерский конструктор ругается и на
консьюмерскую переменную. Переменная в окружении процесса — это заявление
оператора «настройка действует», и оно ложно независимо от того, какую роль
процесс играет. Значение не разбирается: пустое `KAFKAX_CONSUMER_READ_TIMEOUT=`
— такое же заявление, как и заполненное.

Отставлено семнадцать ключей — ровно те ключи v1, которых в v3 нет под прежним
именем. Полноту списка сторожит тест `TestEveryV1KeyIsLiveOrRetired`, и записи
не удаляются с выходом следующей мажорной версии: ценность отказа в том, что он
встречает конфиг, пролежавший в репозитории годы.

| Отставленный ключ | Чем заменён |
|---|---|
| `clientid` | `client_id`. В v1 у поля не было тега `yaml`, и ключом становилось имя поля в нижнем регистре. Переменная `KAFKAX_CLIENT_ID` не менялась и остаётся живой |
| `security_protocol` | Замены нет: протокол выводится из самих настроек — TLS включён, когда заполнена секция `tls` (или передан `WithTLSConfig`), SASL — когда задан `sasl.mechanism`. **Самый опасный ключ списка:** конфиг, говоривший `SASL_SSL`, но без секции `tls`, соединялся бы открытым текстом, и не сообщил бы об этом никто |
| `tls.identification_algorithm` | Замены нет: Go всегда проверяет hostname, отказаться можно только через `tls.insecure_skip_verify`. Для сертификата с чужим именем — `tls.server_name` |
| `producer.batch_timeout` | `producer.linger`. Это была та же ручка под вторым именем; в v1 умолчания различались (`1s` против `0s`), так что победитель зависел от того, какой ключ был заполнен |
| `producer.batch_size` | Замены нет: он ограничивал батч **числом записей**, а franz-go ограничивает только размером. Есть `producer.batch_bytes`, он был и в v1 |
| `producer.message_queue_size` | Замены нет: собственной очереди перед клиентом у продюсера больше нет. Буфер клиента ограничивают `producer.max_buffered_records` и `producer.max_buffered_bytes` |
| `producer.inactive_worker_ttl` | Замены нет: воркеров по тенантам не существует |
| `producer.cleanup_worker_interval` | Замены нет: сборщика этих воркеров тоже |
| `consumer.message_queue_size` | `consumer.message_queue_batches` — и единица измерения сменилась: v1 буферизовал отдельные сообщения, v3 — целые батчи опроса |
| `consumer.handler_max_retries` | `consumer.handler_retries` — и смысл нуля инвертирован относительно v1: там значение считало вызовы обработчика, а `0` означал «повторять бесконечно»; здесь `0` — «не повторять», `N` — `N` повторов сверх первого вызова, `-1` — бесконечно |
| `consumer.enable_auto_commit` | Замены нет: v3 всегда коммитит отмеченное и отмечает запись только после `nil` от обработчика. Это гарантия пакета, а не настройка; как часто отмеченное уходит брокеру, задаёт `consumer.commit_interval` |
| `consumer.max_poll_interval` | `consumer.rebalance_timeout`. Оба ограничивают ожидание участника перед исключением из группы, но v3 тратит бюджет на колбэк ребаланса, а не на паузу между опросами |
| `consumer.socket_timeout` | `dial_timeout` в корне: бюджет на установку TCP/TLS-соединения с брокером |
| `consumer.read_timeout` | Замены нет: v1 читал по сообщению и ограничивал этот вызов. v3 читает батчами и блокируется до их прихода; сколько брокер держит fetch в ожидании данных, задаёт `consumer.max_wait` |
| `consumer.read_error_backoff` | Замены нет: пауза после неудачного опроса принадлежит franz-go и наружу не вынесена. `consumer.handler_retry_delay` — другое: это пауза между повторами обработчика |
| `consumer.inactive_worker_ttl` | Замены нет: партиционный воркер живёт ровно столько, сколько партиция назначена этому консьюмеру |
| `consumer.cleanup_worker_interval` | Замены нет по той же причине |

Имя переменной окружения у каждого — `KAFKAX_` плюс путь ключа в верхнем
регистре: `consumer.socket_timeout` → `KAFKAX_CONSUMER_SOCKET_TIMEOUT`.
Исключение одно — `clientid`, у которого менялось только имя в файле.
...
| `NewKafkaProducer(ctx, cfg)` | `NewProducer(cfg, opts ...Option)` — контекст конструктору не нужен |
| `NewKafkaConsumer(cfg)` | `NewConsumer(cfg, opts ...Option)` |
| `producer.Close()` без результата | `Close() error`. Читайте его: `ErrFlushIncomplete` означает потерянные сообщения |
| `consumer.Stop()` без результата | `Stop() error` |
| `consumer.SubscribeAll()` | Метода нет: подписка происходит внутри `Start` |
| `PublishRequest.TenantID` | Поля нет: изоляция теперь по партициям, а партицию выбирает `Key` |
...
#### Что покажет загрузка конфигурации

Все семнадцать отставленных ключей — это ключи v1, так что перенесённый файл
или карта переменных окружения не запустятся, пока их не разберут. Сверх того
валидация v3 отвергает две комбинации, которые v1 не проверял:

* `sasl.mechanism: PLAIN` без TLS. В v1 это был штатный `SASL_PLAINTEXT`, в v3
  — ошибка: пароль уходит брокеру открытым текстом. Выход — включить TLS,
  перейти на `SCRAM`, либо записать решение в конфигурацию через
  `sasl.allow_plaintext: true`;
* `producer.required_acks: 1` (умолчание v1) при `enable_idempotence: true`
  (умолчание и там, и там). Идемпотентная запись определена только для
  `acks=-1`, поэтому перенесённая единица приезжает претензией валидации с
  указанием на поле, а не отказом создания клиента изнутри franz-go и не молча
  отключённой идемпотентностью.

#### Что не покажет никто

Здесь и лежит настоящий риск переноса: имена и типы совпадают, конфигурация
валидна, компилятор молчит.

| Что | v1 | v3 | Чем грозит |
|---|---|---|---|
| Исход исчерпанных повторов | сообщение **пропускалось**, оффсет коммитился | партиция встаёт на **паузу**, оффсет не двигается | Конвейер, который в v1 ехал дальше через потерю, в v3 остановится. Это осознанный выбор, см. [«Почему пауза, а не пропуск»](#почему-пауза-а-не-пропуск); вернуть прежнее поведение — `WithSkipHook`, возвращающий `nil` |
| `producer.max_retries`, умолчание | `3` | `-1`, без ограничения | Разобрано ниже |
| `producer.required_acks`, умолчание | `1` — достаточно лидера | `-1` — все реплики ISR | Запись стала надёжнее и медленнее; на теме с просевшим числом синхронных реплик отправка начнёт отказывать там, где ответ лидера считался бы успехом. Явную единицу, наоборот, отвергнет валидация — см. выше |
| `producer.max_inflight`, умолчание | `1` | `5` | Действует только при `enable_idempotence: false`; при умолчании `true` значение не применяется вовсе |
| `consumer.max_bytes`, умолчание | `10485760` | `52428800` | Ответ на fetch впятеро крупнее — учитывайте вместе с `message_queue_batches` при расчёте памяти |
| `consumer.max_wait`, умолчание | `250ms` | `500ms` | Пустой fetch держится вдвое дольше |
| Ёмкость очереди воркера, умолчание | `1000` **сообщений** | `100` **батчей** | Старый ключ роняет старт, но конфиг, который его не задавал, переезжает молча — а сто батчей на партицию — это до `100 × MaxPartitionBytes` резидентной памяти, порядка 100 МиБ на умолчаниях |
...
v2.0.0 — это уже franz-go, те же типы, те же гарантии и те же умолчания.
Ломающих изменений три, и **молча не меняется ничего**: ни одно умолчание
между v2.0.0 и v3 не сдвинулось.

**1. Путь модуля** — `github.com/alfzs/kafkax/v3`. Компилятор.

**2. Два поля `ConsumerConfig` переименованы.**

| Было (v2.0.0) | Стало (v3) |
|---|---|
| `MessageQueueSize`, yaml `message_queue_size`, env `KAFKAX_CONSUMER_MESSAGE_QUEUE_SIZE` | `MessageQueueBatches`, yaml `message_queue_batches`, env `KAFKAX_CONSUMER_MESSAGE_QUEUE_BATCHES` |
| `HandlerMaxRetries`, yaml `handler_max_retries`, env `KAFKAX_CONSUMER_HANDLER_MAX_RETRIES` | `HandlerRetries`, yaml `handler_retries`, env `KAFKAX_CONSUMER_HANDLER_RETRIES` |

Смысл и умолчания не менялись: в v2.0.0 первое поле уже считало батчи (умолчание
`100`), а ноль во втором уже означал «без повторов» (умолчание `0`). **Значения
переносятся как есть.** `Config`, собранный литералом в Go, поправит компилятор;
yaml-файл и переменные окружения — отставленные ключи, отказом на старте. Текст
отказа объясняет смену единицы измерения и инверсию нуля применительно к v1 — к
переходу с v2.0.0 эти две части не относятся, к переходу с неё относится только
само имя.

**3. Пять полей уехали из `Config` в опции конструктора.**

| Было (v2.0.0) | Стало (v3) |
|---|---|
| `cfg.Logger` | `WithLogger(logger)` |
| `cfg.TLSConfig` | `WithTLSConfig(tlsCfg)` |
| `cfg.ExtraOpts` | `WithExtraOpts(opts...)` |
| `cfg.OnPanic` | `WithPanicHook(hook)` — только консьюмеру |
| `cfg.OnMessageSkipped` | `WithSkipHook(hook)` — только консьюмеру |

Компилятор: полей больше нет. Сигнатуры хуков прежние, но теперь у них есть
имена — `PanicHook` и `SkipHook`. `NewProducer`, `NewConsumer` и `Validate`
принимают `opts ...Option`, поэтому вызовы без опций компилируются без правки.
Подробности — [«Опции конструктора»](#опции-конструктора).

Один сдвиг заметен только в логах: сводку поведения печатает больше не
`Config.LogValue`, а сами конструкторы — записью `options` при успешном
создании клиента. Признаки `tls_config_set` и `extra_opts` сохранили имена,
`on_panic_set` и `on_message_skipped_set` стали `panic_hook_set` и
`skip_hook_set` — по именам опций, которыми хуки теперь задаются, — и пишутся
только консьюмеру: продюсеру хуки задать нельзя.
...
> нельзя**: тот же срез получит каждый повтор и затем хук `WithSkipHook`.
> Нужна модификация — копируйте. Включение `kgo.WithPools` через
> `WithExtraOpts` не поддерживается.
...
Рапорт о ней — стек в лог, `kafkax.consumer.panics{site="handler"}`, вызов хука
`WithPanicHook` — пишется **один раз на сообщение, а не на попытку**. Иначе при `-1`
...
хука `WithSkipHook` — опции конструктора, а не поля конфигурации (см.
...
consumer, err := kafkax.NewConsumer(cfg, kafkax.WithSkipHook(
    func(ctx context.Context, msg kafkax.IncomingMessage, cause error) error {
        // Хук зовут и во время shutdown, когда исходный ctx уже отменён, —
        // а спасать сообщение надо именно тогда.
        dlqCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), 5*time.Second)
        defer cancel()

        return dlq.Publish(dlqCtx, msg, cause) // nil ⇒ оффсет двигается
    }))
```
...
| Потеря недопустима, но зависать в обработчике нельзя | `HandlerRetries: N`, конструктор без `WithSkipHook` — партиция встаёт на паузу, инцидент разбирает дежурный. **Это умолчание** |
| Прогресс важнее отдельного сообщения | `HandlerRetries: N` + `WithSkipHook` с записью в DLQ и возвратом `nil` — единственный вариант, в котором конвейер не останавливается никогда |
...
| `skipped` | **сдвинулся** | повторы исчерпаны, хук `WithSkipHook` вернул `nil` — сообщение забрал потребитель пакета |
...
| `KafkaLogLevel` | `KAFKAX_KAFKA_LOG_LEVEL` | `info` | Порог логов самого franz-go: `debug`, `info`, `warn`, `error`, `none`. Только ужесточение — уровень логгера, переданного `WithLogger`, остаётся внешним фильтром |
...
**Полей без `yaml` и `env` в `Config` нет ни одного.** Логгер, готовый
`*tls.Config`, опции franz-go и оба пользовательских хука — это поведение, а не
данные, и задаются они опциями конструктора: см.
[«Опции конструктора»](#опции-конструктора).
...
Способов продолжить три: включить `TLS.Enabled` (или передать `WithTLSConfig`),
...
`Config.Validate(opts ...Option)` проверяет обе секции сразу — для приложения,
создающего из одного `Config` и продюсер, и консьюмер. Конструкторы вызывают
проверку только своей роли: продюсеру незачем требовать `consumer.group`,
консьюмеру — `producer.flush_timeout`. Поэтому `Config`, прошедший
`NewProducer`, может не пройти `Validate`.

Опции она принимает те же, что и конструкторы, и по той же причине, по которой
их принимает сборка клиента: `WithTLSConfig` — часть ответа на вопрос «будет ли
соединение зашифровано», и без него проверка `PLAIN` отвергала бы полностью
рабочую конфигурацию с mTLS из памяти. Роль здесь обе сразу, поэтому
`WithPanicHook` и `WithSkipHook` `Validate` принимает, хотя `NewProducer` их и
отвергнет.
...
`Config` — это данные: всё, что можно записать в yaml-файл или в переменную
окружения, и ничего сверх того. Поведение клиента — логгер, готовый
`*tls.Config`, аварийный выход в опции franz-go и два пользовательских хука —
живыми значениями в файле не бывает, поэтому задаётся отдельно, опциями
конструктора:
...
| Опция | Роль | Что задаёт |
|---|---|---|
| `WithLogger(*slog.Logger)` | обе | Логгер библиотеки. Без опции — `slog.Default()`. Логи franz-go идут туда же, но не ниже `KafkaLogLevel` |
| `WithTLSConfig(*tls.Config)` | обе | Готовый `*tls.Config`. Имеет приоритет над всей секцией `TLS`. Нужен для mTLS с ротацией, кастомного `VerifyPeerCertificate`, сертификатов из памяти. Валидация судит о том же соединении: с этой опцией `PLAIN` считается защищённым и без `TLS.Enabled` |
| `WithExtraOpts(...kgo.Opt)` | обе | Опции franz-go: приписываются последними и побеждают всё, что вывела библиотека. Аварийный выход, не замена конфигурации. `kgo.WithPools` не поддерживается — пакет не вызывает `Record.Recycle` |
| `WithPanicHook(PanicHook)` | только консьюмер | Реакция на восстановленную панику в горутине библиотеки: `site` типа `kafkax.PanicSite` (`PanicSiteHandler`, `PanicSiteProcessMessage`, `PanicSitePartitionWorker`, `PanicSitePollLoop`, `PanicSiteMessageSkipped`, `PanicSitePanicHook`), `recovered`, `stack`. Синхронный — не должен блокироваться |
| `WithSkipHook(SkipHook)` | только консьюмер | Судьба сообщения, исчерпавшего повторы. Вызывается синхронно в горутине партиционного воркера — партиция стоит, ребаланс ждёт; `ctx` к этому моменту может быть уже отменён, поэтому для записи в DLQ используйте `context.WithoutCancel` с собственным таймаутом. См. [«Политика повторов»](#политика-повторов) |

Интерфейс `Option` закрыт неэкспортируемым методом: реализовать его снаружи
нельзя, и набор законных значений — ровно эти пять конструкторов. Повторный
вызов заменяет прежнее значение; исключение — `WithExtraOpts`, у которого список
накапливается.

### Опция не той роли — отказ, а не тихий пропуск

У продюсера нет ни собственных горутин, ни обработчиков, ни отравленных
сообщений, поэтому оба хука к нему неприменимы. `NewProducer` с таким хуком не
принимает его молча, а возвращает `ErrInapplicableOption`:

```
kafkax: option is not applicable to this role: WithSkipHook applies to the consumer, but was passed to the producer constructor
```

Молчание здесь было бы худшим исходом: `WithSkipHook`, переданный продюсеру,
выглядит как настроенная выдача отравленных сообщений в DLQ, а на деле не делает
ничего — и узнали бы об этом не при старте, а когда сообщение потребуется
спасать.

`nil` вместо опции — тоже отказ, `ErrNilOption`: почти всегда это результат
вызова, вернувшего `nil` вместо `Option`, и молчаливый пропуск потерял бы
настройку без следа.

```
kafkax: option must not be nil: option #0
```

Обе ошибки — ошибки вызова, а не конфигурации: они возвращаются до всякой
проверки полей и в агрегат `ErrInvalidConfig` не входят.
