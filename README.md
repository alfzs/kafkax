...
валидация v3 отвергает две комбинации, законные в v1:
...
* `producer.required_acks: 1` (умолчание v1) при `enable_idempotence: true`
  ...
  `acks=-1`, поэтому явно перенесённая единица приезжает претензией валидации,
  а не молча отключённой идемпотентностью.
...
| `producer.required_acks`, умолчание | `1` — достаточно лидера | `-1` — все реплики ISR | Запись стала надёжнее и медленнее; на теме с просевшим числом синхронных реплик отправка начнёт отказывать там, где v1 отвечал успехом |
