...
type ConsumerConfig struct {
	...
	// вырос бы в MaxPollRecords раз. Поэтому прежний ключ не игнорируется, а
	// отвергается: конфигурация с message_queue_size в файле или с
	// KAFKAX_CONSUMER_MESSAGE_QUEUE_SIZE в окружении не запускается, а ошибка
	// объясняет смену единицы измерения.
	//
	...
	// ключ не игнорируется, а отвергается: конфигурация с handler_max_retries
	// в файле или с KAFKAX_CONSUMER_HANDLER_MAX_RETRIES в окружении не
	// запускается, а ошибка объясняет инверсию.
	//
	...
	//
	// Что происходит после исчерпания повторов, определяет хук WithSkipHook, и
