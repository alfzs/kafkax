...
//     отдельно от контекста и этой веткой не затрагивается. Обещания «худший
//     случай равен MessageTimeout» он, вопреки прежней редакции этого
//     комментария, не держит — и не держит его вообще никто: оба срока
//     привязаны к первой записи батча и оба упираются в один запрет. При
//     идемпотентном продюсере franz-go не заваливает батч, судьба которого
//     неизвестна (sink.go, canFail), поэтому истечение срока констатируется и
//     откладывается, пока партиция недоступна. Измерено: при MessageTimeout=5s
//     вызов не вернулся и через 150s; снятие RecordDeliveryTimeout при этом
//     ничего не меняет — мутация эквивалентная. Подробности и цифры в
//     docs/audit/09-mutation-sweep.md, честная формулировка контракта — в doc.go.
...
// некого.
//
// Декремент стоит под тем же RLock, что и проверка closing в acquire, и это не
// формальность. Close выставляет closing под write-замком, взять который он не
// может, пока держится наш RLock, — поэтому увиденное здесь closing=true
// означает, что флаг стоял ещё до входа в release, а значит все принятые
// отправки успели увеличить счётчик раньше нашего декремента. Ноль под этим
// замком окончателен. Вынести декремент наружу — и сигнал «ждать больше
// некого» смог бы обогнать отправку, которая проверку closing уже прошла:
// Close вернулся бы и закрыл клиент у неё под руками.
func (p *Producer) release() {
	p.mu.RLock()
	defer p.mu.RUnlock()

	if p.inflight.Add(-1) != 0 || !p.closing {
		return
	}
