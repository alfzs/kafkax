...
type loadRunner struct {
	...
	// acked — число подтверждённых брокером отправок. Пишется горутиной
	// нагрузки, читается после закрытия done, поэтому без синхронизации.
	acked int
}
	...
	go func() {
		...
		for n := 0; ; n++ {
			...
			err := producer.SendMessage(t.Context(), kafkax.PublishRequest{
				Topic: topic,
				Value: []byte(fmt.Sprintf("load-%d", n)),
			})
			if err == nil {
				load.acked++
			}
