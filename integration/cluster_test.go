...
func clusterLeaderFailover(t *testing.T, cluster *kafkaCluster) {
	...
	// самом окне перевыборов, наблюдается потеря — примерно в трети прогонов
	// ровно одна подтверждённая запись в теме отсутствует, а одна
	// НЕподтверждённая в ней лежит. Механизм лежит ниже kafkax (продюсер
	// пакета — прямой ProduceSync без собственных очередей), измерения и
	// открытый вопрос описаны в docs/audit/09-mutation-sweep.md. Утверждать
	// здесь то, что стек не держит, значило бы завести мигающий тест;
	// утверждать это про окно после перевыборов — можно, и именно оно
