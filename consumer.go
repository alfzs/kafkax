...
// в каждую попытку повтора (Consumer.HandlerRetries) и затем в
// хук WithSkipHook — уже после возврата из ProcessMessage. Обработчик,
...
//
// Отдельно про WithExtraOpts: kgo.WithPools, протащенный туда, включает в
...
// пути повторов (Consumer.HandlerRetries), а исчерпав их — попадёт в
// хук WithSkipHook либо, если он не задан, остановит свою партицию на
